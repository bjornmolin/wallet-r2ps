...
public class R2psRequestController {
    ...
    if (r2psResponse.get().httpStatus() != HttpStatus.OK.value()) {
      ...
      try {
        ServiceRequestHandlingException serviceRequestHandlingException =
            objectMapper.readValue(r2psResponse.get().serviceResponseJws(),
  ...
  public ResponseEntity<AsyncResponseDto<String>> service(
    ...
    if (stateJws == null) {
      ...
      ECKey ecKey = new com.nimbusds.jose.jwk.gen.ECKeyGenerator(com.nimbusds.jose.jwk.Curve.P_256)
          .keyID("example-key-id")
          .generate();

      ECPrivateKey privateKey = ecKey.toECPrivateKey();

      stateJws = generateInitialDeviceHsmStateJws(deviceId.toString(), walletId.toString(), clientPublicKey.toPublicJWK(), privateKey);

    }

    UUID requestId = UUID.randomUUID();
    R2psRequest r2psRequest =
        new R2psRequest(requestId, walletId, deviceId, stateJws, serviceRequestJws);
